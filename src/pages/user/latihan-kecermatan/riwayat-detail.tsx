...
export default function RiwayatLatihanKecermatanDetail() {
  ...
  const stats = useMemo(() => {
    ...
    listHistory.list.forEach((item: any) => {
        ...
        if (!kiasanId) return;
        if (!kiasanGroups[kiasanId]) kiasanGroups[kiasanId] = [];
        kiasanGroups[kiasanId].push(item);
    });

    const kiasanIds = Object.keys(kiasanGroups).sort((a, b) => Number(a) - Number(b));
    const totalColumns = configKiasans.length > 0 ? configKiasans.length : kiasanIds.length;
    ...
    // Use configKiasans order if available
    const kiasanList = configKiasans.length > 0 ? configKiasans : kiasanIds.map(id => ({ id }));
    
    kiasanList.forEach((kiasan, idx) => {
        const group = kiasanGroups[kiasan.id] || [];
        const correctInColumn = group.filter((item: any) => item.jawaban === item.soalLatihanKecermatan?.jawaban).length;
