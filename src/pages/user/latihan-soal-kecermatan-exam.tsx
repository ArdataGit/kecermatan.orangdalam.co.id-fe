...
import { Button, Dialog, Select, Switch } from 'tdesign-react';
...
export default function LatihanSoalKecermatanExam() {
  ...
  const calculateStats = () => {
      ...
      const getHankerCategory = (score: number) => {
           ...
           if (score >= 50) return { label: "Rendah", color: "text-orange-600", bg: "bg-orange-50", border: "border-orange-200", desc: "Ketahanan kerja Anda rendah. Penurunan tempo dan ketelitian cukup besar di bagian tengah atau akhir. Stamina cepat tergerus dan output menurun signifikan seiring waktu.", saran: "Kurangi beban dari awal dengan tempo yang tidak terlalu tinggi. Latih ketahanan dengan target durasi yang panjang, mulai dari yang ringan. Atur pola makan dan tidur agar kondisi fisik mendukung konsentrasi lebih lama." };
           return { label: "Sangat Rendah", color: "text-red-600", bg: "bg-red-50", border: "border-red-200", desc: "Ketahanan kerja Anda sangat rendah. Penurunan drastis sejak awal atau tengah, menunjukkan stamina mudah habis dan konsentrasi cepat terpecah.", saran: "Perbaiki kondisi fisik dan mental secara bertahap. Mulai dengan durasi kerja pendek yang dapat diselesaikan dengan baik, lalu tingkatkan perlahan. Jaga pola istirahat dan konsumsi gizi yang baik agar energi lebih tahan lama." };
      };
      ...
      const getFinalCategory = (score: number) => {
          ...
          if (score >= 50) return { label: "Rendah", color: "text-orange-700", bg: "bg-orange-100", border: "border-orange-300", desc: "Hasil akhir masih rendah. Mulai dari perbaikan dasar dan tingkatkan bertahap. Hasil biasanya cepat terlihat." };
          return { label: "Sangat Rendah", color: "text-red-700", bg: "bg-red-100", border: "border-red-300", desc: "Hasil akhir masih di bawah target. Fokus pada ritme dan konsistensi. Kenaikan akan lebih mudah jika dilakukan rutin." };
      };
      ...
      return (
          <div className="w-full max-w-screen-2xl mx-auto p-4 md:p-8 font-['Poppins']">
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 mb-6 text-center">
                  <h2 className="text-3xl font-bold mb-8 text-[#000000] text-center">Hasil Latihan Kecermatan</h2>
                                           ...
                                           return (
                                               <tr key={key} className="hover:bg-gray-50 transition-colors">
                                                   ...
                                                   <td className="px-6 py-4 whitespace-nowrap text-center">
                                                       <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${ans.isCorrect ? 'bg-orange-100 text-[#ffb22c]' : 'bg-red-100 text-red-600'}`}>{ans.isCorrect ? 'Benar' : 'Salah'}</span>
                                                   </td>
                                               </tr>
                                           );
                                       })}
  ...
  const symbols = currentKiasan.kiasan; 
               ...
               <div className="flex flex-wrap gap-3 justify-center bg-gray-50 p-6 rounded-xl border border-dashed border-gray-300">
                   <AnimatePresence mode='wait'>
                        <motion.div key={`${currentKiasanIndex}-${currentSoalIndex}`} initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} transition={{ duration: 0.05, ease: "linear" }} className="flex gap-4 md:gap-8">
                            {(Array.isArray(currentSoal.soal) ? currentSoal.soal : []).map((char: string, idx: number) => (<div key={idx} className="font-bold text-4xl md:text-5xl text-gray-800">{char}</div>))}
                        </motion.div>
                   </AnimatePresence>
               </div>
           </div>
