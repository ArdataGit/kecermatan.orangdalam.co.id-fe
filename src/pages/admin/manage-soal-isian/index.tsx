...
import ManageIsian from './manage';
...
export default function ManageSoalIsian() {
  ...
  const handleDeleted = async (id: number) => {
    FetchAPI(deleteData(`admin/kategori-soal-isian/remove/${id}`)).then(
      () => {
        getData.refresh();
      }
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return <span>{row?._count?.soalIsian || 0}</span>;
      },
    },
    {
        ...
        return (
          <div className="flex justify-center gap-5">
            ...
            <Button
              ...
              className="ml-2"
              onClick={() => {
                navigate(`/manage-soal-isian/${row.id}/history`);
              }}
            >
              <IconClock size={14} />
            </Button>
  ...
  return (
    <section className="">
      ...
      <BreadCrumb
        page={[{ name: 'Bank Soal Isian', link: '/manage-soal-isian' }]}
      />
