...
export default function HistoryRankingKecermatanAdmin() {
  const { id } = useParams();
  const navigate = useNavigate();

  const getData = useGetList({
    url: 'admin/kategori-soal-kecermatan/history',
    initialParams: {
    ...
    },
    {
        ...
        cell: ({ row }: any) => (
            <Tooltip content="Lihat Detail Pengerjaan">
                <Button shape="circle" theme="primary" variant="text" onClick={() => navigate(`/manage-soal-kecermatan/${id}/history/${row.user?.id || row.userId}`)}>
                    <IconEye size={18} />
                </Button>
            </Tooltip>
        )
