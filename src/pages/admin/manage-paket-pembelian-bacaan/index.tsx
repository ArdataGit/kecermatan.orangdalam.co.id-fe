...
export default function UserIndex() {
  ...
  const getList = useGetList({
    url: 'admin/paket-pembelian-bacaan/get',
    initialParams: {
      skip: 0,
      take: 10,
      sortBy: 'createdAt',
  ...
  const handleDeleted = async (id: number) => {
    FetchAPI(deleteData(`admin/paket-pembelian-bacaan/remove/${id}`)).then(
      () => {
        getList.refresh();
      }
    );
  ...
  const columns = [
...

    {
      ...
      width: 300,
      cell: ({ row }: any) => {
        return <span>{row?.kategoriSoalBacaan?.judul_kategori}</span>;
      },
    },
    {
      title: 'Tipe',
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <span className="text-center">{((row?.kategoriSoalBacaan?.waktu || 0) / 60).toFixed(0)} Menit</span>
        );
      },
    },
    {
          ...
          <div className="flex justify-center gap-5">
            <Button
              ...
              onClick={() => {
                setDetail(() => ({
                  ...row,
                  paketLatihanId: row.kategoriSoalBacaanId,
                }));
                setVisible(true);
              }}
            >
