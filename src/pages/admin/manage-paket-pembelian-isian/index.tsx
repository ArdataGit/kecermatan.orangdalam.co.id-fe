...
export default function ManagePaketPembelianIsian() {
  ...
  const getList = useGetList({
    url: 'admin/paket-pembelian-isian/get',
    initialParams: {
      skip: 0,
      take: 10,
      sortBy: 'createdAt',
  ...
  const handleDeleted = async (id: number) => {
    FetchAPI(deleteData(`admin/paket-pembelian-isian/remove/${id}`)).then(
      () => {
        getList.refresh();
      }
    );
  ...
  const columns = [
...

    {
      ...
      width: 300,
      cell: ({ row }: any) => {
        return <span>{row?.kategoriSoalIsian?.judul_kategori}</span>;
      },
    },
    {
      title: 'Tipe',
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <span className="text-center">{row?.kategoriSoalIsian?._count?.soalIsian || 0}</span>
        );
      },
    },
    {
          ...
          <div className="flex justify-center gap-5">
            <Button
              ...
              onClick={() => {
                setDetail(() => ({
                  ...row,
                  paketLatihanId: row.kategoriSoalIsianId,
                }));
                setVisible(true);
              }}
            >
