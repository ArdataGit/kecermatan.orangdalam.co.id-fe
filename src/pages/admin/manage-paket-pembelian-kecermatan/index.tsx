...
export default function UserIndex() {
  ...
  const getList = useGetList({
    url: 'admin/paket-pembelian-kecermatan/get',
    initialParams: {
      skip: 0,
      take: 10,
      sortBy: 'createdAt',
  ...
  const handleDeleted = async (id: number) => {
    FetchAPI(deleteData(`admin/paket-pembelian-kecermatan/remove/${id}`)).then(
      () => {
        getList.refresh();
      }
    );
  ...
  const columns = [
...

    {
      ...
      width: 300,
      cell: ({ row }: any) => {
        return <span>{row?.kategoriSoalKecermatan?.judul_kategori}</span>;
      },
    },
    {
      title: 'Tipe',
      ...
      cell: ({ row }: any) => {
        return (
          <span>
            ...
            {row.type === 'KECERMATAN' && 'Kecermatan'}
          </span>
        );
      },
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <span className="text-center">{((row?.kategoriSoalKecermatan?.waktu || 0) / 60).toFixed(0)} Menit</span>
        );
      },
    },
    {
          ...
          <div className="flex justify-center gap-5">
            <Button
              ...
              onClick={() => {
                setDetail(() => ({
                  ...row,
                  paketLatihanId: row.kategoriSoalKecermatanId,
                }));
                setVisible(true);
              }}
            >
