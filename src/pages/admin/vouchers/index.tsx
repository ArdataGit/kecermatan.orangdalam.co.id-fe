...
export default function VoucherIndex() {
  ...
  const paketPembelian = useGetList({
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <span className="text-center">
            {row.VoucherProduct?.length ? row.VoucherProduct?.length : 'Semua'}{' '}
            Product{' '}
          </span>
        );
      },
                ...
                setDetail(() => ({
                  ...
                  harga:
                    row.tipePotongan === 'HARGA'
                      ...
                      : '',
                  voucherProduct: row?.VoucherProduct?.map((item: any) => ({
                    label: item?.paketPembelian?.nama,
                    value: item?.paketId,
                  })),
                }));
                setVisible(true);
