...
function removeEmptyAttrs(obj) {
  ...
  Object.keys(obj).forEach(function (key) {
    if (!isUndefined.isUndefined_1(obj[key]) || isNull.isNull_1(obj[key])) {
      newObj[key] = obj[key];
    }
  });
  return newObj;
...
function getBackgroundColor(color) {
  if (isString.isString_1(color)) {
    return color;
  }
  if (isArray.isArray_1(color)) {
...
function getCharacterLength(str, maxCharacter) {
  var hasMaxCharacter = isNumber.isNumber_1(maxCharacter);
  if (!str || str.length === 0) {
    if (hasMaxCharacter) {
      return {
        length: 0,
...
function pxCompat(param) {
  return isNumber.isNumber_1(param) ? "".concat(param, "px") : param;
}
