...
export function isMathJaxVersion3(MathJax) {
    return (MathJax != null && typeof MathJax == 'object' && 'version' in MathJax && typeof MathJax.version == 'string' &&
...
export function isMathJaxVersion2(MathJax) {
    return (MathJax != null && typeof MathJax == 'object' && 'Hub' in MathJax);
}
...
export async function renderEquation(equation, element, engine = 'katex', lazyLoad, display = false, preview = false, previewUid = '', previewClassName = [], katexRenderOptions = {}) {
    var _a;
    if (engine == 'mathjax') {
        ...
        if (lazyLoad != null) {
            try {
                (_a = global.window).CKEDITOR_MATH_LAZY_LOAD ?? (_a.CKEDITOR_MATH_LAZY_LOAD = lazyLoad());
                element.innerHTML = equation;
                await global.window.CKEDITOR_MATH_LAZY_LOAD;
                await renderEquation(equation, element, engine, undefined, display, preview, previewUid, previewClassName, katexRenderOptions);
            }
            catch (err) {
                element.innerHTML = equation;
                console.error(`math-tex-typesetting-lazy-load-failed: Lazy load failed: ${String(err)}`);
            }
        }
        else {
            element.innerHTML = equation;
            console.warn(`math-tex-typesetting-missing: Missing the mathematical typesetting engine (${String(engine)}) for tex.`);
        }
    }
}
